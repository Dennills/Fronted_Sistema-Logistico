...
import { ShieldCheck, Clock, Search, RefreshCw, Package, TrendingUp, Filter } from 'lucide-react';
...
export default function VerificacionGuias() {
  ...
  const fetchGuias = useCallback(async () => {
    ...
    setErrorGlobal('');
    try {
      // GET /api/guias/?rolid=1 — disponible para Administrador
      const res = await getGuias({ rolid: user?.rolid || 1 });
      const raw = res.data;
      // Log temporal para ver la estructura real de la API
      console.log('📦 Respuesta API /api/guias/:', JSON.stringify(raw, null, 2));
      // La API puede devolver un array directamente o un objeto { guias: [...] }
      const list = Array.isArray(raw) ? raw : (raw?.guias || raw?.data || []);
      if (list.length > 0) {
        console.log('🔑 Claves de una guía:', Object.keys(list[0]));
        console.log('🔑 Claves del contenedor:', list[0].contenedor ? Object.keys(list[0].contenedor) : 'contenedor es null/undefined');
      }
      setGuias(list.map((g) => ({ ...g, _verificando: false })));
    } catch (err) {
      console.error('Error cargando guías:', err);
      if (err.code === 'ERR_NETWORK' || err.code === 'ECONNREFUSED') {
        setErrorGlobal('No se puede conectar con el servidor. Verifica que FastAPI esté corriendo (uvicorn main:app --reload).');
      } else if (err.response?.status === 403) {
        setErrorGlobal('Tu rol no tiene permiso para ver esta sección.');
      } else {
        setErrorGlobal(`Error del servidor: ${err.response?.data?.detail || err.message}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleVerify = async (guiaid) => {
    ...
    try {
      await verificarGuia(guiaid, user?.id, user?.rolid || 1);
      setGuias((prev) => prev.map((g) =>
        g.id === guiaid ? { ...g, _verificando: false, vehiculoverificado: true, contenedorverificado: true } : g
      ));
    } catch (err) {
      console.error(err);
      setGuias((prev) => prev.map((g) => g.id === guiaid ? { ...g, _verificando: false } : g));
      alert(`Error al verificar: ${err.response?.data?.detail || err.message}`);
    }
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          ...
          <div className="mt-2 w-full bg-slate-100 rounded-full h-1">
            <div className="bg-amber-400 h-1 rounded-full transition-all" style={{ width: totalGuias ? `${(totalPendientes / totalGuias) * 100}%` : '0%' }} />
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          ...
          <div className="mt-2 w-full bg-slate-100 rounded-full h-1">
            <div className="bg-emerald-500 h-1 rounded-full transition-all" style={{ width: totalGuias ? `${(totalVerificadas / totalGuias) * 100}%` : '0%' }} />
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            {['todos', 'pendiente', 'verificado'].map((f) => (
              <button key={f} onClick={() => setFiltroEstado(f)}
                className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all capitalize ${filtroEstado === f ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400'}`}
              >{f}</button>
            ))}
          </div>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="py-3 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:table-cell">Empresa ID</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="6" className="py-14 text-center text-slate-400 text-sm">No se encontraron guías.</td></tr>
                ...
                <tr key={g.id} className={`hover:bg-slate-50/80 transition-colors ${g._verificando ? 'opacity-50' : ''}`}>
                  ...
                  <td className="py-4 px-5 hidden md:table-cell text-sm text-slate-700">{g.empresaid}</td>
                  <td className="py-4 px-5">
                    {(() => {
                      // Intentamos todas las variantes posibles de nombre que la API puede usar
                      const num =
                      ...
                      return num ? (
                        ...
                        <span className="text-xs text-slate-400 italic">
                          {g.contenedor ? 'Ver consola' : 'Sin contenedor'}
                        </span>
                      );
                    })()}
                  </td>
